...
if uploaded_file:
    ...
    if "result" in st.session_state:
        ...
        if "annotated_image" in result:
            ...
            # Count violations as number of people who are not compliant
            persons = result.get("persons", [])
            violations = sum(
                1
                for person in persons
                if person.get("compliance", "").lower() != "compliant"
            )
            ...
            if violations > 0:
                ...
                for person in result.get("persons", []):
                    ...
                    if status != "compliant":
                        with st.expander(
                            f"Person ID: {person.get('person_id', 'N/A')} - Status: {status.upper()}"
                        ):
